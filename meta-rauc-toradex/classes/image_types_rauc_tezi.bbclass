# This class implements a Rauc-enabled Toradex Easy Installer image
# Based largely on meta-mender-community/meta-mender-toradex-nxp/classe/image_type_mender_tezi.bbclass

IMAGE_TYPEDEP:rauc_tezi:append = " wic.gz ext4 "
...
do_image_rauc_tezi[recrdeptask] += "do_deploy"
RM_WORK_EXCLUDE += "${PN}"

def rootfs_rauc_tezi_emmc(d):
...
python rootfs_rauc_tezi_json() {
    ...
    data["version"] = "Rauc %s" % d.getVar('DISTRO_CODENAME')
    ...
    data["blockdevs"] = rootfs_rauc_tezi_emmc(d)
...
IMAGE_CMD:rauc_tezi () {
    ...
    ${IMAGE_CMD_TAR} --transform='s/.*\///' \
		     ...
		     -chf ${IMGDEPLOYDIR}/${IMAGE_NAME}.rauc_tezi.tar \
    ...
    ln -sf ${IMAGE_NAME}.rauc_tezi.tar ${IMGDEPLOYDIR}/${IMAGE_LINK_NAME}.rauc_tezi.tar
}
do_image_rauc_tezi[dirs] += "${WORKDIR}/image-json ${DEPLOY_DIR_IMAGE}"
do_image_rauc_tezi[cleandirs] += "${WORKDIR}/image-json"
do_image_rauc_tezi[prefuncs] += "rootfs_rauc_tezi_json"
IMAGE_TYPEDEP:rauc_tezi[vardepsexclude] = "SRCDATE"
